...
interface ResultsViewProps {
  ...
  isParetoOnly?: boolean; // New prop for simplified view
  onUnlockFullStudy?: () => void; // Action to exit simplified view
}
...
`;
  ...
  return (
    <div className="w-full max-w-4xl mx-auto space-y-8 animate-fade-in pb-12">
      ...
      {/* Actions Toolbar - Hidden on Print and HIDDEN in Pareto Mode */}
      {!isParetoOnly && (
      ...
      <div id="printable-guide">
        ...
        <div className={`bg-white rounded-xl paper-shadow p-8 border-t-4 ${isParetoOnly ? 'border-red-500' : 'border-indigo-500'} print:shadow-none print:border-0 print:border-t-0 print:mb-6`}>
            ...
            {/* Advance Organizer / Pareto Essence */}
        ...
        {/* UNLOCK FULL STUDY CALL TO ACTION (Visible in Pareto Mode) */}
        {isParetoOnly && onUnlockFullStudy && (
            <div className="mt-8 bg-white p-8 rounded-xl border-2 border-dashed border-gray-200 text-center animate-fade-in">
                ...
                <p className="text-gray-500 mb-6 max-w-lg mx-auto">
                    O NeuroStudy gerou a essência. Para um estudo ativo completo com quiz e slides, mude o modo para "Normal" ou "Turbo".
                </p>
                <button 
                    onClick={onUnlockFullStudy}
                    className="bg-indigo-600 text-white px-8 py-3 rounded-xl font-bold hover:bg-indigo-700 shadow-lg shadow-indigo-200 transition-transform active:scale-95 flex items-center gap-2 mx-auto"
                >
        ...
        {/* Checkpoints Timeline (HIDDEN IN PARETO MODE) */}
        {!isParetoOnly && (
      ...
      {/* Footer / Instructions - Hide on Print AND Hide in Pareto Mode */}
      {!isParetoOnly && (
